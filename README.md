...
- **デプロイURL:** `https://sueCalystegia00.github.io/nostr-client/`
- **トリガー:** `main` ブランチへのpush、または GitHub Actions の手動実行 (`workflow_dispatch`)
- **CI チェック:** デプロイ前にBiomeによるリント・フォーマットチェックおよびTypeScriptの型チェックが自動実行されます。チェックに失敗した場合、デプロイは実行されません。
